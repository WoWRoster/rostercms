...
{
	$dm_query1 = "SELECT * FROM `" . $roster->db->table('user_link','user') . "` WHERE `uid` = '".$row['id']."'";
	$dm_result1 = $roster->db->query($dm_query1);
	$row1 = $roster->db->fetch($dm_result1);
	$cc = $roster->db->num_rows($dm_result1);
	$tooltip = array();
	...
	$roster->tpl->assign_block_vars('user', array(
		...
		'NAME'      => $row['usr'],
		'TOOLTIP'   => makeOverlib(implode("<br>", $tooltip), $row['usr'], '', 1, '', ''),
		'EMAIL'     => $row['email'],
