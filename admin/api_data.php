...
if (isset($_POST['process']) && $_POST['process'] == 'process')
{
	...
	if (isset($_POST['truncate']))
	{
		switch($_POST['truncate'])
		{
			...
			case 'api_lmh':
				$qenchant = "TRUNCATE TABLE `" . $roster->db->table('api_lmh') . "`;";
				$resultitem = $roster->db->query($qenchant);
...
$roster->tpl->assign_block_vars('sections', array(
	...
	$queryx = "SELECT * FROM `" . $roster->db->table('api_lmh') . "` ORDER BY `idx` desc;";
	$resultx = $roster->db->query($queryx);
	$lmh = array();
	while ($row = $roster->db->fetch($resultx))
	{
		//$usage[$row['date']][$row['type']]['total']=$row['total'];
		$lmh[$row['type']][] = array('Last-Modified'=>$row['Last-Modified']);
	}
	//echo '<pre>';
	//print_r($lmh);
	//echo '</pre>';
