...
class rostersync {
	...
	*/
	function _getMembersToUpdate( $where = false )
	{
		...
		$query =	"SELECT members.member_id, members.name, guild.guild_id, guild.guild_name, members.server, guild.region ".
		"FROM `".$roster->db->table('members')."` members LEFT JOIN `".$roster->db->table('guild')."` guild ON members.guild_id = guild.guild_id " .
		"LEFT JOIN `". $roster->db->table('updates',$addon['basename']). "` updates ON members.member_id = updates.member_id ".
