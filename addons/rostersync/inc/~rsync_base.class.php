...
class rsyncBase {
	...
	*/
	...
	function _getCharacterInfo()
	{
			...
			{
//	"rangedDmgMin":3044.0,    "rangedDmgMax":4070.0,    "rangedSpeed":2.622,    "rangedDps":1356.5209,    "rangedCrit":18.545528,"rangedCritRating":1456,    "rangedHitPercent":7.32669,    "rangedHitRating":880
...
//    "spellPower":102,    "spellPen":0,    "spellCrit":8.121372,    "spellCritRating":1456,    "spellHitPercent":8.589913,"spellHitRating":880, 
				...
				*/
				...
				if ($item['artifactId'] == '0')
				{
					if (isset($item['tooltipParams']['gem0']))
					{
						...
						$this->gemx['gem0']['gem_bonus'] = ($g0['itemSubClass'] == 11 ? $g0['name'] : $roster->db->escape($g0['gemInfo']['bonus']['name']) );
					...
					if (isset($item['tooltipParams']['gem1']))
					{
						...
						$this->gemx['gem1']['gem_bonus'] = ($g1['itemSubClass'] == 11 ? $g1['name'] : $roster->db->escape($g1['gemInfo']['bonus']['name']) );//$roster->db->escape($g1['gemInfo']['bonus']['name']);
					...
					if (isset($item['tooltipParams']['gem2']))
					{
						...
						$this->gemx['gem2']['gem_bonus'] = ($g2['itemSubClass'] == 11 ? $g2['name'] : $roster->db->escape($g2['gemInfo']['bonus']['name']) );//$roster->db->escape($g2['gemInfo']['bonus']['name']);
						$this->gemx['gem2']['gem_color'] = strtolower($g2['gemInfo']['type']['type']);
						$gem_c++;
					}
					if (isset($item['tooltipParams']['gem3']))
					{
						$gem3 = $item['tooltipParams']['gem3'];
						$g3 = $roster->api2->fetch('item',array('id'=>$item['tooltipParams']['gem3']));
						$this->gemx['gem3'] = $g3;
						$this->gemx['gem3']['Tooltip'] = $roster->db->escape($g3['name']."<br>".$g3['gemInfo']['bonus']['name']."<br>".$g3['description']);
						$this->gemx['gem3']['gem_bonus'] = ($g3['itemSubClass'] == 11 ? $g3['name'] : $roster->db->escape($g3['gemInfo']['bonus']['name']) );//$roster->db->escape($g2['gemInfo']['bonus']['name']);
						$this->gemx['gem3']['gem_color'] = strtolower($g3['gemInfo']['type']['type']);
						$gem_c++;
					}
					$this->_insertGems();
				...
				$this->data["Equipment"][$slot]['GemCount'] = $gem_c;
