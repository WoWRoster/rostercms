...
// new function for voting      
      ...
      function vote( $id, $vote ){
            ...
            $query = "SELECT * FROM `".$roster->db->table('ss',$addon['basename'])."` WHERE `id` ='".$id."'";
            ...
            $query1 = "UPDATE `".$roster->db->table('ss',$addon['basename'])."` SET `votes` = '".$count."' , `rateing` = '".$rate."' WHERE `id` ='".$id."'";
...
// end new function      
      ...
      */
      ...
      $error = '';
      $old_path = $addon['dir'].'screenshots/'.$old.'/'.$file;
      $new_path = $addon['dir'].'screenshots/'.$new.'/'.$file;
      if (file_exists($old_path))
            {
                  if (!file_exists($new_path))
                        ...
                        {
                              $this->setMessage( '<span class="red">ERROR RG:001a-CC - Selected dir "'.$new_path.'" does not exist</span>' );
                              $error = '1';
                        }
            }
            else
            {
                  $this->setMessage( '<span class="red">ERROR RG:001b-CC - Selected dir "'.$old_path.'" does not exist</span>' );
                  $error = '1';
            }
            if ($error != '1')
            {
                  $query = "UPDATE `".$roster->db->table('ss',$addon['basename'])."` SET `catagory` = '".$new."' WHERE `id` = '".$id."'";
                  $result = $roster->db->query($query) or die_quietly($roster->db->error(),'Database Error',basename(__FILE__),__LINE__,$query);
      ...
      global $roster, $addon;
            $query = "UPDATE `".$roster->db->table('ss',$addon['basename'])."` SET `approve` = 'YES' WHERE `id` ='".$approve."'";
            $result = $roster->db->query($query) or die_quietly($roster->db->error(),'Database Error',basename(__FILE__),__LINE__,$query);
	...
	function deletesc( $delete )
	{
		...
		$query5 = "SELECT * FROM `".$roster->db->table('ss',$addon['basename'])."` WHERE id='".$delete."' ";
		...
		$dir = $addon['dir'].'screenshots/';
		$filename = $row['file'].'-image.'.$row['ext'];
		...
		else
		{
			if( file_exists($dir.$filename) )
			{
				if( unlink($dir.$filename) && unlink($dir.$filename2))
				{
					...
					$roster->db->query("DELETE FROM `".$roster->db->table('ss',$addon['basename'])."` WHERE id='$delete' ");
					return TRUE;
				}
				else
				{
