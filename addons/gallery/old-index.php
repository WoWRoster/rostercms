...
$vop = isset($_GET['op']);
      ...
      if ($id != '' && $vop == 'vote' && $addon['config']['rg_use_votepopup'] == '0')
      ...
      if( $roster->auth->getAuthorized($addon['config']['config_' . $row['catagory']]) )
	{
            ...
            if ($caption != '')
            {
                  ...
                  <th class="simpleborderheader '.$addon['config']['rg_tn_bc'].'borderheader" align="center" valign="top">'.$caption.'</th>';
                        ...
                        if ($addon['config']['rg_use_votepopup'] == '1')
                        {
                              echo '<th><a href="'.makelink('&amp;id='.$row['id'].'').'" >'.$roster->locale->act['vote'].'</a></th>';
                        }
                        if ($addon['config']['rg_use_votepopup'] == '0')
                        {
                              echo '<th><a href="'.makelink('&amp;id='.$row['id'].'&amp;op=vote').'" onclick="return popitup(\''.makelink('&amp;id='.$row['id'].'&amp;op=vote').'\')">'.$roster->locale->act['vote'].'</a></th>';
                        }
