...
class forum
{
	...
	function getCrumbsb($topic)
	{
		...
		/*
		$loc = '<a href="'.makelink('guild-'.$addon['basename'].'').'">Forums</a> > 
		<a href="'.makelink('guild-'.$addon['basename'].'-forum&amp;id=' . $row['forum_id']).'">'.$row['forum'].'</a> > 
		<a href="'.makelink('guild-'.$addon['basename'].'-topic&amp;tid=' . $row['topic_id']).'">'.$row['topic'].'</a>';
		*/
		$loc = '<ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="'.makelink('guild-'.$addon['basename'].'').'">Forums</a></li>
			<li class="breadcrumb-item"><a href="'.makelink('guild-'.$addon['basename'].'-forum&amp;id=' . $row['forum_id']).'">'.$row['forum'].'</a></li>
			<li class="breadcrumb-item active"><a href="'.makelink('guild-'.$addon['basename'].'-topic&amp;tid=' . $row['topic_id']).'">'.$row['topic'].'</a></li>
		</ol>';
	...
	function getCrumbsa($topic)
	{
		...
		$loc = '<ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="'.makelink('guild-'.$addon['basename'].'').'">Forums</a></li>
			<li class="breadcrumb-item active"><a href="'.makelink('guild-'.$addon['basename'].'-forum&amp;id=' . $row['forum_id']).'">'.$row['forum'].'</a></li>
		</ol>';
		return $loc;
	}
	...
	*/
		...
		*/
		function processLock( $id , $mode )
		{
			...
			$query1 = "UPDATE `" . $roster->db->table('posts',$addon['basename']) . "` SET `locked` = '$mode' WHERE `topic_id` = '".$id."';";
			...
			{
				$roster->set_message($roster->locale->act['t_locked_' . $mode] );
			}
			return true;
		}
		function processSticky( $id , $mode )
		{
			global $roster, $addon, $installer;

			$query = "UPDATE `" . $roster->db->table('topics',$addon['basename']) . "` SET `sticky` = '$mode' WHERE `topic_id` = '".$id."';";
			$result = $roster->db->query($query);
			...
			{
				$roster->set_message($roster->locale->act['t_sticky_' . $mode] );
			}
			return true;
		}
		function processAnounc( $id , $mode )
		{
			global $roster, $addon, $installer;

			$query = "UPDATE `" . $roster->db->table('topics',$addon['basename']) . "` SET `anounc` = '$mode' WHERE `topic_id` = '".$id."';";
			$result = $roster->db->query($query);
			...
			{
				$roster->set_message($roster->locale->act['t_anounc_' . $mode] );
			}
			return true;
		}
		/*
			delete topic
		*/
		function topic_delete( $id , $mode )
		{
			global $roster, $addon, $installer;

			$query1 = 'DELETE FROM `' . $roster->db->table('topics',$addon['basename']) . "` WHERE `topic_id` = '" . $id . "';";
			$result1 = $roster->db->query($query1);
			$query1 = 'DELETE FROM `' . $roster->db->table('posts',$addon['basename']) . "` WHERE `topic_id` = '" . $id . "';";
			$result1 = $roster->db->query($query1);
		}
	...
	*/
	...
	function get_add_access($id,$type)
	{
		global $roster, $addon;
		$table = '';
		$p_table = '';
		...
		$query = "SELECT * FROM `" . $roster->db->table($p_table,$addon['basename']) . "` WHERE `".$p_id."` ='".$id."'";
		$result = $roster->db->query($query) or die_quietly($roster->db->error(),'Database Error',basename(__FILE__),__LINE__,$query);
		$row = $roster->db->fetch($result);
	...
	{
		global $roster;
		if ($roster->auth->getAuthorized( $access ))
		{
			return true;
		}
		else
		{
			$roster->tpl->set_filenames(array(
				'noaccess' => $addon['basename'] . '/no_access.html',
			));

			echo $roster->tpl->display('noaccess');
