...
$forums = $functions->getForums();
	...
	foreach($forums as $id => $forum)
	{
		...
		{
			$roster->tpl->assign_block_vars('forum.subforum', array(
...
$roster->tpl->set_handle('forum',$addon['basename'] . '/admin_forum2.html');
...
//`forum_id`  `parent_id`  `title`  `access` `access_post` `order_id`  `desc`  `misc`  `active`  `locked`  `icon`
