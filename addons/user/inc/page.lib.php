...
class userPage extends user
{
		...
		else
		{
			...
			var oAuth2AuthWindow;
				jQuery(document).ready( function($){
					...
					jQuery('#charupdate').click(function(e)
					{
						e.preventDefault();
						//alert('boo');
						oAuth2AuthWindow = window.open('".$auth_url."&state=claim', 'masheryOAuth2AuthWindow', 'width=430,height=660');
					});
			...
			while( $row = $roster->db->fetch($result) )
			{
				...
				$e = $row['thumbnail'];
				$c = explode("-", $e);
				$type=array('inset','profilemain','thumbnail');

				foreach($type as $t)
				{
				//http://render-api-us.worldofwarcraft.com/static-render/us/hellscream/74/113337162-inset.jpg
				if ($t=='insert')
				{
					$img_url = "http://render-api-us.worldofwarcraft.com/static-render/us/".$c[0]."-".$t.".jpg?alt=/wow/static/images/2d/inset/".$row['race']."-".$row['gender'].".jpg";
				}
				else
				{
					$img_url = "http://us.battle.net/static-render/us/".$c[0]."-".$t.".jpg";
				}
				//echo $img_url.'<br>';
				//$save_path = $addon['dir'].'x/'.$name.'-'.$server.'.jpg';
				$save_path = 'chars/'.$row['name'].'-'.$row['realm'].'-'.$t.'.jpg';
	...
	function realmsPage()
	{
		...
		//		die_quietly($roster->db->error, 'claim alt', __FILE__,__LINE__,$query);
			...
			while( $row = $roster->db->fetch($result) )
			{
				$realm[$row['realm']] = array(
					'name'			=>	$row['realm'],
					'count'			=> (isset($realm[$row['realm']]['count']) ? $realm[$row['realm']]['count']+1 : 1 ),
					'battlegroup'	=> $row['battlegroup'],
				);
			}
			//d($realm);
			foreach($realm as $r => $d)
			{
				$roster->tpl->assign_block_vars('urealms', array(
					'NAME'		=> $r,
					'CHARS'		=> $d['count'],
					'BG'		=> $d['battlegroup'],					
				));
