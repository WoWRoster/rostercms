...
var oAuth2AuthWindow;
	...
	function openWin()
	{
		//oAuth2AuthWindow = window.open('".$auth_url."&stage=2&state=claim', 'masheryOAuth2AuthWindow', 'width=430,height=660');
		location.href = '".makelink('user-user-alt&stage=2')."';
	}
...
if (isset($_GET['stage']) && $_GET['stage'] == 2)
{
	...
	if (is_array($chars['characters']))
	{
		...
		$update_sql[] = "UPDATE `" . $roster->db->table('user_members') . "` SET `access` = '".$mranks."' WHERE `id` = '" . $roster->auth->user['id'] . "'";
		//echo '<pre>';
		//print_r($mranks);
		//exit;
		...
		location.href = '".makelink('user-user-alt&stage=3')."';
