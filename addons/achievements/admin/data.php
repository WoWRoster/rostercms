...
if (isset($_POST['process']) && $_POST['process'] == 'process' OR $_GET['process'] == true)
{
	...
	$querystra = "SELECT * FROM `" . $roster->db->table('achie', $addon['basename']) . "`;";
	$resulta = $roster->db->query($querystra);
	$classr = $roster->db->num_rows($resulta);
	
	$querystrb = "SELECT * FROM `" . $roster->db->table('crit', $addon['basename']) . "`;";
...
$menu .= $config->buildConfigMenu('rostercp-addon-' . $addon['basename']);
	...
	$roster->tpl->set_filenames(array('acbody' => $addon['basename'].'/admin.html'));
$body = $roster->tpl->fetch('acbody');
