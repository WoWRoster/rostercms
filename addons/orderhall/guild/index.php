...
else
{
	...
	$data = file_get_contents( $addon['dir'] . 'guild/oh1.json' );
	...
	function recursiveArrayToList(Array $array = array())
	{
	  ...
	  foreach ($array as $key =>$value) {
			...
			{
				echo '<li><img src="'.$addon['image_url'].''.( in_array($value['id'], $a['quests']) ? 'complete' : 'incomplete' ).'.gif"></a> <a href="http://www.wowhead.com/quest='.$value['id'].'" target="_blank" >' . $value['name'] . '</a>';
				if (isset($value['id1']) && $value['id1']!= '')
				{
					echo ' - <img src="'.$addon['image_url'].''.( in_array($value['id1'], $a['quests']) ? 'complete' : 'incomplete' ).'.gif"></a> <a href="http://www.wowhead.com/quest='.$value['id1'].'" target="_blank" >' . $value['name1'] . '</a>';
				}
				if (isset($value['id1']) && $value['id2']!= '')
				{
					echo ' - <img src="'.$addon['image_url'].''.( in_array($value['id2'], $a['quests']) ? 'complete' : 'incomplete' ).'.gif"></a> <a href="http://www.wowhead.com/quest='.$value['id2'].'" target="_blank" >' . $value['name2'] . '</a>';
				}
				if (isset($value['id3']) && $value['id3']!= '')
				{
					echo ' - <img src="'.$addon['image_url'].''.( in_array($value['id3'], $a['quests']) ? 'complete' : 'incomplete' ).'.gif"></a> <a href="http://www.wowhead.com/quest='.$value['id3'].'" target="_blank" >' . $value['name3'] . '</a>';
				}
		...
		if (isset($value['next']) && is_array($value['next'])) {
		   recursiveArrayToList($value['next']);
		}
	  }
	  echo '</ul>';
	}
	...
	recursiveArrayToList($oh[$a['class']]);
	echo '</div>';
	/*$f = array();
