...
roster_add_js($js, 'inline', 'header', false, false);
	...
	function prepareData( $query, $where, $group, $order_first, $order_last, $fields, $listname )
	{
		...
		*/
		...
		$t_a = explode( ',', $get_s);
		foreach ($t_a as $ei => $fv)
		{
			list($cf,$cv) = explode(':', $fv);
			$this->active_filters[$cf]=$cv;
		}
		...
		foreach ( $this->fields as $field => $DATA )
		{
			...
			if ( isset($DATA['float']) )
			{
				$float = ' SortTable-label--'.$DATA['float'];
			}
			$f_sort = false;
			if ( isset($this->active_filters[$field]) )
			{
				$f_sort = true;
			}
			$roster->tpl->assign_block_vars('header_cell',array(
				...
				'NAME'		=> $field,
				'IS_SORT'	=> ( $f_sort ? ' is-sorted'.($this->active_filters[$field]=='d' ? '-reverse' : '') : ''),
				'FLOAT'		=> $float,
				)
			);
	...
	function assignRow( $class, $member_id, $type, $cells )
	{
		...
		foreach( $cells as $cell )
		{
			$roster->tpl->assign_block_vars($class . '.cell',array(
				...
				'FLOAT'   => ( isset($cell['cell_float']) ? $cell['cell_float'] : 'left'),
			));
		}
	}
