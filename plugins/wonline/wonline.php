...
class wonline
{
	...
	*/
	...
	function display ()
	{
		...
		$sql = 'SELECT u.usr, u.user_display, u.id, s.*
		...
		while ($row = $roster->db->fetch($result))
		{
			...
			if ($row['id'] != 0)
			{
				...
				if ($row['id'] != $prev_user_id)
				{
					...
					{
						$user_online_link[] ='<a href="' . makelink('user-user-profile-' . $row['id'] . '&amp;a=c:' . $row['id']) . '">' . $row['user_display'] . '</a> ';
					}
					else
					{
						$user_online_link[] = $row['user_display'];
					}
