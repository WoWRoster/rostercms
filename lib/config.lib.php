...
class roster_config
{
	 ...
	 */
	function roster_config( $tablename, $where='1', $prefix='config_', $form_builder=true )
	{
		global $roster;
		...
		$this->submit_button = "<input type=\"submit\" class=\"cfg-button cfg-button--new\" value=\"" . $roster->locale->act['config_submit_button'] . "\" />\n<input type=\"reset\" class=\"cfg-button cfg-button--new\" name=\"Reset\" value=\"" . $roster->locale->act['config_reset_button'] . "\" onclick=\"return confirm('" . $roster->locale->act['confirm_config_reset'] . "')\"/>\n<input type=\"hidden\" name=\"process\" value=\"process\" />\n";
		$this->form_end = "</form>\n";
		...
		if ($this->form_builder)
		{
			$roster->tpl->assign_vars(array(
				'BEGIN_FORM'		=> $this->form_start,
				'END_FORM'			=> $this->form_end,
				'BOXOPTIONS'		=> $this->submit_button,
			));
		}
		else
		{
	...
	function buildConfigPage()
	{
		...
		foreach($this->db_values['menu'] as $values)
		{
			...
			switch ($type[0])
			{
				...
				case 'blockframe':
					$addpage = true;
					$page .= '<div class="row cfg-header">
								  <div class="col-md-12 col-sm-6">' . ($header_text ? $header_text: '') . $this->buildBlock($values['name']) . '</div></div>';
					break;
	...
	function buildPage($page, $columns)
	{
		...
		if( $columns > 1 )
		{
			$html = '<div class="table-responsive"><table class="table"><tr class="rostercp-nohover"><td valign="top">';
		}
		else
		{
			$html = '';
...
';*/
					$html .= '<div class="row cfg-header"><div class="col-md-12 col-sm-6">' . ($header_text ? $header_text : '') . $this->buildBlock($values['name']) . '</div></div>';
					break;
		...
		foreach($this->db_values[$block] as $values)
		{
			...
			switch ($input_type[0])
			{
				...
				case 'radio':
					...
					$input_field .= '<div class="btn-group" data-toggle="buttons">';
					foreach( $options as $value )
					{
						$vals = explode('^',$value);
						$input_field .= '<label class="btn btn-default btn-xs' . ( $values['value'] == $vals[1] ? ' active' : '' ) . '" id="rad_' . $this->prefix . $this->radio_num . '">
						<input type="radio" id="rad_' . $this->prefix . $this->radio_num . '" name="' . $this->prefix . $values['name'] . '" value="' . $vals[1] . '" ' . ( $values['value'] == $vals[1] ? 'checked="checked"' : '' ) . ' />' . $vals[0] . "</label>\n";
						$this->radio_num++;
					}
					$input_field .= '</div>';
					break;
...
/*
		...
		<tr class="membersRowColor' . (($i%2)+1) . '">
			<td class="membersRowCell">
				<span class="ui-icon ui-icon-help cfg-icon-cp" ' . $this->createInlineTip($values['tooltip'],$values['description']) . '></span>
				' . $values['description'] . '
			</td>
...
<div class="tier-3-a">
	<div class="tier-3-b">
		<div class="config">
			...
			$html .= '<div class="row cfg-row">';
			$html .= '<div class="col-md-6 col-sm-5"><i class="fa fa-exclamation-triangle cfg-icon-cp" ' . $this->createInlineTip($values['tooltip'],$values['description']) . '></i> ' . $values['description'] . '</div>';
			$html .= '<div class="col-md-6 col-sm-5">' . $input_field . '</div>';
			$html .= '</div>';
			$i++;
		}
		return $html;
