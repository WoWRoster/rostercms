<?php
/**
 * Plugin API: roster_hook class
 *
 * we stole this from wordpress they wont mind lol
 */
 ...
 * @see ArrayAccess
 */
final class Roster_Hook implements Iterator, ArrayAccess {
	...
	public function add_filter( $tag, $function_to_add, $priority, $accepted_args ) {
		$idx = _roster_filter_build_unique_id( $tag, $function_to_add, $priority );
	...
	public function remove_filter( $tag, $function_to_remove, $priority ) {
		$function_key = _roster_filter_build_unique_id( $tag, $function_to_remove, $priority );
	...
	public function has_filter( $tag = '', $function_to_check = false ) {
		...
		$function_key = _roster_filter_build_unique_id( $tag, $function_to_check, false );
		if ( ! $function_key ) {
			return false;
		}
	...
	/**
	 * Normalizes filters set up before WordPress has initialized to Roster_Hook objects.
	 *
	 * @since 4.7.0
	 * @access public
	 * @static
	 ...
	 * @return Roster_Hook[] Array of normalized filters.
	 */
	public static function build_preinitialized_hooks( $filters ) {
		/** @var Roster_Hook[] $normalized */
		$normalized = array();

		foreach ( $filters as $tag => $callback_groups ) {
			if ( is_object( $callback_groups ) && $callback_groups instanceof Roster_Hook ) {
				$normalized[ $tag ] = $callback_groups;
				continue;
			}
			$hook = new Roster_Hook();
