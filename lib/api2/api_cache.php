...
class apicache {
	...
	public function InsertICache($data,$par)
	{
		...
		$tooltip = $roster->api2->item->item($data['id'],$data,null,null);//$roster->api->Item->item($data,null,null);
		...
		$update->add_value('bl' , (isset( $par['bl'] ) ? $par['bl'] : '' ) );
	...
	/*
		New Cacheing system
		1 we will place last modified headers for all calls in this table using replace into if not present
		2 we will store all field values and identifing details in this table name server size etc.
		3 we will update lmh when it is out of date
		4 we will place data in the apropreate tables based on calls
	*/
	
	/*
		Function _get_lmh
		var $params @array an array of valued from the api call
		return	true: a row of data forom the table
				false: false to mark no data
	*/
	public function _get_lmh($parameters)
	{
		global $roster;
		
		$par = array(
			'type'			=> $roster->api2->usage['type'],
			'battletag' 	=> ( isset( $parameters['battletag'] ) ? $parameters['battletag'] : ''),
			'id' 			=> ( isset( $parameters['id'] ) ? $parameters['id'] : ''),
			'data' 			=> ( isset( $parameters['data'] ) ? $parameters['data'] : ''),
			'follower'		=> ( isset( $parameters['follower'] ) ? $parameters['follower'] : ''),
			'artisan' 		=> ( isset( $parameters['artisan'] ) ? $parameters['artisan'] : ''),
			'leaderboard' 	=> ( isset( $parameters['leaderboard'] ) ? $parameters['leaderboard'] : ''),
			'name' 			=> ( isset( $parameters['name'] ) ? $parameters['name'] : ''),
			'server' 		=> ( isset( $parameters['server'] ) ? $parameters['server'] : ''),
			'region' 		=> ( isset( $parameters['region'] ) ? $parameters['region'] : ''),
			...
			'bl'			=> ( isset( $parameters['bl'] ) ? $parameters['bl'] : ''),
		);
		$querystr = "SELECT * FROM `" . $roster->db->table('api_lmh') . "` WHERE ". $roster->db->build_query('SELECT', $par) . ";";
		$result = $roster->db->query($querystr);
		$lines = $roster->db->num_rows($result);
		if ($lines == 1)
		{
			return $roster->db->fetch($result);
		}
		else
		{
			return false;
	...
	public function put_lmh($data, $parameters)
	{
		global $roster;
		$x = array();
		//$x = $params;
		...
		$x['Last-Modified'] = ( isset($data['lastModified']) ? ( $data['lastModified']/1000 ) : time() );
		$query = "REPLACE INTO `" . $roster->db->table('api_lmh') . "` SET " . $roster->db->build_query('UPDATE', $x).";";// . " WHERE `type` = '".$method."' AND `url` = '".$url."'";
	...
	public function CacheCheck($id,$parameters)
	{
		...
		$par = array(
			'context'		=> ( isset( $parameters['context'] ) ? $parameters['context'] : ''),
			'bl'			=> ( isset( $parameters['bl'] ) ? $parameters['bl'] : ''),
			'locale'		=> $roster->config['api_url_locale'],
		);
		...
		{
			$p .= ' AND `bl` = "'.$parameters['bl'].'" ';
		}		
		$sql = "SELECT * FROM `" .$roster->db->table('api_items') . "` WHERE `item_id` = '".$id."' AND ". $roster->db->build_query('SELECT', $par) . ";";
		...
		if ($roster->db->num_rows($result) == 0)
		{
			...
			else
			{
				...
				{
					$sql = "DELETE FROM `" .$roster->db->table('api_gems') . "` WHERE `gem_id` = '".$id."' ".$p."";
					$result = $roster->db->query($sql);
					return false;
				}
			}
		...
		else
		{
			...
			{
				$sql = "DELETE FROM `" .$roster->db->table('api_items') . "` WHERE `item_id` = '".$id."' AND ". $roster->db->build_query('SELECT', $par) . ";";
				$result = $roster->db->query($sql);
				return false;
			}
		}
	...
	public function guild($parameters,$usage)
	{
		return;// $this->_cachecheck($parameters['id']);
	}
	public function insertguild($data,$vars,$parameters)
	{
		return;//$this->_insertcache($data,$vars,$parameters);
	}

	public function account($parameters,$usage)
	{
		return;//$this->_insertcache($data,$vars,$parameters);
	}
	public function insertaccount($data,$vars,$parameters)
	{
		return;//$this->_insertcache($data,$vars,$parameters);
	}
