		...
		foreach( $talentBuildData as $build => $talentData )
		{
			...
			foreach ($talentData['Talents'] as $t_name => $info )
			{
				...
				else
				{
					...
					$this->add_ifvalue($info["Name"], 'tooltip');
				}

				if( !empty($info['icon']) )
				{
					$this->add_value('texture', $this->fix_icon($info['icon']));
				}

				$tree_pointsspent++;

				$this->add_value('row', $info['row']);
				$this->add_value('talent_id', $t_name);
				$this->add_value('column', $info['column']);
